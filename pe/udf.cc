/*
        lib_mysqludf_sys - a library with miscellaneous (operating) system level
   functions Copyright (C) 2007  Roland Bouman Copyright (C) 2008-2009  Roland
   Bouman and Bernardo Damele A. G. web: http://www.mysqludf.org/ email:
   mysqludfs@gmail.com, bernardo.damele@gmail.com
...
DLLEXP
my_bool lib_mysqludf_sys_info_init(UDF_INIT *initid, UDF_ARGS *args,
                                   char *message);

DLLEXP
void lib_mysqludf_sys_info_deinit(UDF_INIT *initid);

DLLEXP
char *lib_mysqludf_sys_info(UDF_INIT *initid, UDF_ARGS *args, char *result,
 ...
 */
DLLEXP
my_bool sys_get_init(UDF_INIT *initid, UDF_ARGS *args, char *message);

DLLEXP
void sys_get_deinit(UDF_INIT *initid);

DLLEXP
char *sys_get(UDF_INIT *initid, UDF_ARGS *args, char *result,
              unsigned long *length, char *is_null, char *error);
 ...
 */
DLLEXP
my_bool sys_set_init(UDF_INIT *initid, UDF_ARGS *args, char *message);

DLLEXP
void sys_set_deinit(UDF_INIT *initid);

DLLEXP
long long sys_set(UDF_INIT *initid, UDF_ARGS *args, char *is_null, char *error);
...
DLLEXP
my_ulonglong sys_exec(UDF_INIT *initid, UDF_ARGS *args, char *is_null,
                      char *error);
...
my_bool lib_mysqludf_sys_info_init(UDF_INIT *initid, UDF_ARGS *args,
  ...
  my_bool status;
  if (args->arg_count != 0) {
    strcpy(message, "No arguments allowed (udf: lib_mysqludf_sys_info)");
    status = 1;
  } else {
    status = 0;
  }
...
my_bool sys_set_init(UDF_INIT *initid, UDF_ARGS *args, char *message) {
  if (args->arg_count != 2) {
    strcpy(message, "Expected exactly two arguments");
    return 1;
  }
  ...
  if ((initid->ptr = (char *)malloc(args->lengths[0] + 1 + args->lengths[1] +
                                    1)) == NULL) {
...
long long sys_set(UDF_INIT *initid, UDF_ARGS *args, char *is_null,
                  char *error) {
